...
def index():
    ...
    # create visuals
    # Top 10 categories
    count_categories = np.sum(df.iloc[:,4:].values, dtype=np.float64, axis=0)
    top_category_count = pd.Series(count_categories, df.columns[4:]).sort_values(ascending=False)[:10]
